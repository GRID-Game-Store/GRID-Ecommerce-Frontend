import {
  ...
  ResponseGameRandom,
} from "@/app/components/main/types/Response";
...
import {
  ...
  ItemSmallRow,
} from "@/app/components/shared/Item/variants/item";
import {
  fireEvent,
  getByAltText,
  getByText,
...
describe("check all item variation", () => {
  it("should check ItemSmallRow render", () => {
    ...
    expect(container.querySelector("img")?.getAttribute("width")).toBe(
      widthItem,
    );
    expect(container.querySelector("p")?.innerHTML).toBe(gameMock.title);
    expect(container.querySelector("span")?.innerHTML).toBe(
      gameMock.genres[0].name,
    );
    expect(
      getByText(container, getPrice(gameMock), { selector: "button" }),
    ).toBeDefined();
    expect(
      getByText(container, getPrice(gameMockWithFreeProduct), {
        selector: "button",
      }),
    ).toBeDefined();
  });
  ...
  it("should check ItemLargePreview render", () => {
    const { container, debug } = render(
      <ItemLargePreview game={gameFullInfoMock} />,
    );
    expect(container.querySelector("img")?.src).toBe(
      gameFullInfoMock.game_media.banner_url,
    );
    expect(container.querySelector("p")?.innerHTML).toBe(
      gameFullInfoMock.title,
    );
    expect(container.querySelector("span")?.innerHTML).toBe("Singleplayer");
    fireEvent.mouseOver(
      getByText(container, getPrice(gameFullInfoMock), { selector: "button" }),
    );
    expect(container.querySelector("video")?.src).toBe(
      gameFullInfoMock.game_media.trailer,
    );
    ...
    expect(
      getByText(container, `Release Date : ${gameFullInfoMock.release_date}`),
    ).toBeDefined();
    expect(
      getByText(container, `Developer & Publisher : Larian Studios`),
    ).toBeDefined();
    expect(
      getByText(container, getPrice(gameFullInfoMock), { selector: "button" }),
    ).toBeDefined();
  ...
  it("should check Items render default variant", () => {
    const { container, debug } = render(
      <Items game={gameFullInfoMock} setActiveHover={() => console.log(1)} />,
    );
    expect(container.querySelector("img")?.src).toBe(
      gameFullInfoMock.cover_image_url,
    );
    expect(container.querySelector("p")?.innerHTML).toBe(
      gameFullInfoMock.title,
    );
  ...
  it("should check Items render variant is row", () => {
    const { container, debug } = render(
      <Items
        game={gameFullInfoMock}
        variant="row"
        setActiveHover={() => console.log(1)}
      />,
    );
    expect(container.querySelector("img")?.src).toBe(
      gameFullInfoMock.cover_image_url,
