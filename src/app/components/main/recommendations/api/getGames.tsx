...
export const getAllGamesBySorting = async ({ pageParam = 0 }, search: string) =>  {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}games?page=${pageParam}&size=23&${search}`)
    const game = res.json()
    return game;
}
