...
export const RevertIcon = () => {
  return (
    <>
      <circle className="checkmark-circle" cx="25" cy="25" r="25" fill="none" stroke="none" />
...
const getStatusColor = (dataStatus: number, statusAnimation: string, isRevert: boolean) => {
...
const StatusIcon = ({ status, data, isRevert = false }: { status: string; data: any, isRevert?: boolean }) => {
  const isSuccess = status === "complete" && data.status === 200 && !isRevert ? true : false;
