...
export const Info: React.FC<IInfoProps> = ({fullInfo}) => {
    ...
    const {isSuccess, data, isFetched, isLoading, refetch} = useAddGameToCartQuery(fullInfo.id)
    ...
    const firstGenre = fullInfo.genres && fullInfo.genres[0] ?  fullInfo.genres[0].name : ""
    const secondGenre = fullInfo.genres && fullInfo.genres[1] ?  fullInfo.genres[1].name : ""   
    ...
    const BuyButtonState = () => {
        ...
        else if(isFetched || isLoading) return  {
            message: "Loading...",
            disabled: true
        }
        else return  {
    ...
    return <Box ml={ml} mt={mt} mr={mr}>
        ...
        <Typography fontWeight={"600"} variant="h6">Release date: {fullInfo.releaseDate} </Typography>
        ...
        <Typography fontWeight={"600"} variant="h6">{`Genre : ${genres}`}</Typography>
        {fullInfo.tags && <ListTagsOrGenres mt='0px' ml='-5px' spacing={0} arrayElements={fullInfo.tags.sort().slice(0, 4)}/>}
        {fullInfo.permitAge && <Rating permit_age={fullInfo.permitAge}/>}
        <Button disabled={BuyButtonState().disabled} onClick={onAddToCart} sx={{width:"100%", fontSize:"20px"}}>{BuyButtonState().message}</Button>
    </Box>
}
