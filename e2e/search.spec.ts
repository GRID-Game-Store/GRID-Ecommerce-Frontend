...
test('write in search game and check in list', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'Battlefield™ 2042' })).toHaveText('Battlefield™ 2042');
});
...
test('write in search and click on name of game', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Battlefield™ 2042' })).toHaveText('Battlefield™ 2042');
});
...
test('write with help keyboard', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Search' }).hover();
    await expect(page.getByRole('link', { name: 'Baldurs Gate 3' })).toHaveText('Baldurs Gate 3');
...
test('search game with help # symbol', async ({ page }) => {
    ...
    const allGameInList =  (await page.locator('.MuiBox-root .mui-ve3r9o div:nth-child(2) > div > div').all()).length - 1 
