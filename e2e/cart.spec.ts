...
test("adding game to cart", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /[0-9]/ }).first().click();
  await page
    .getByRole("button", { name: "Successfully added to cart!" })
    .waitFor();
  await page.getByRole("button", { name: "CART" }).first().click();
  await page.waitForURL(/cart/);
  await page.getByRole("button", { name: "DELETE ALL" }).waitFor();
  expect(
    await page.getByRole("button", { name: "DELETE ALL" }).isVisible(),
  ).toBe(true);
  expect(await page.getByRole("button", { name: "BUY ALL" }).isVisible()).toBe(
    true,
  );
